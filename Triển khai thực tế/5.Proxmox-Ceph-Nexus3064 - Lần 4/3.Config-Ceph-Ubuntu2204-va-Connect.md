...
#### 1. Tạo cặp khóa SSH trên Node [Monitor Daemon] (gọi nó là Node quản trị ở đây) và copy nó cho mỗi Node trong cụm. Do mình muốn xóa hết các cặp khóa cũ nên mình sử dụng ``rm -rf ~/.ssh/*`` để xóa tất cả các file trong thư mục ``~/.ssh``, điều này sẽ xóa hết các khóa SSH và các file cấu hình.
