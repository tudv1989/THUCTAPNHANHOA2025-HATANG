...
Trên Cephnode134 chúng ta thao tác tạo pool tên ceph-pool-from134-for-131:
...
Tiếp theo sẽ đến phần tạo VM HA - Các bạn bỏ qua đoạn testing dưới đây

#### Kiểu 2: Từ đoạn này trở đi chỉ là test thêm , Tạo connect theo dạng block device, Type Storage Directory với ceph client, dùng làm gì cũng được , image RBD =100 (ceph-pool-from134-for-131-rbd)
...
#### Tạo LVM thử - chỗ này là mình test thử thôi - Mặc định LVM đã chặn cấu hình block device name /dev/rbdxxx theo rules LVM ở cuối dòng config của LVM
