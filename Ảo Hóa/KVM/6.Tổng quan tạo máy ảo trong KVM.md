...
Phần này mình dumpxml từ 1 máy ảo nên hơi dài, để ý sửa tên file disk qcow2 của VM ``ubuntu22.04-virsh.qcow2`` và id của VM ``id=``,``<uuid>`` cũng cần thay đổi tránh trùng lặp
...
```
  ...
  2. Định nghĩa VM từ file XML:
     ...
     ```bash
     qemu-img create -f qcow2 /var/lib/libvirt/images/ubuntu22.04-virsh.qcow2 30G
     virsh define ubuntu22.04-virh.xml
     virsh start ubuntu2204-virsh
     ```
